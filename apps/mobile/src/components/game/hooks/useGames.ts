import { Game, GameRepositoryFindAllDto, GameRepositoryService } from "@repo/wrapper/server";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { ExtendedUseQueryResult } from "@/util/types/ExtendedUseQueryResult";

interface FindAllGamesByIdsDto extends Partial<GameRepositoryFindAllDto> {}

export function useGames(dto: FindAllGamesByIdsDto, keepPreviousData = false): ExtendedUseQueryResult<Game[]> {
    ...
    return {
        ...useQuery({
            queryKey: queryKey,
            queryFn: () => {
                if (dto == undefined || dto.gameIds == undefined || dto.gameIds.length === 0) {
                    return null;
                }
