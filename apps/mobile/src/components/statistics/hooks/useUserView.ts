...
export function useUserView(sourceId: string | number, sourceType: FindOneStatisticsDto.sourceType) {
    ...
    const viewMutation = useMutation({
        mutationFn: async () => {
            if (lastRegisteredViewSourceId.current && lastRegisteredViewSourceId.current === sourceId) {
                return;
            }
