...
export function useMatomoTracker() {
  ...
  const [hasAcceptedCookies, setHasAcceptedCookies] = useLocalStorage<boolean>({
    key: "cookie-consent",
    defaultValue: false,
    ...
    notifications.show({
      ...
      message: (
        <Stack className={"w-full items-start"}>
          <Text>
            We use cookies and similar storage methods to keep track of your
            session and how you are using our website.
          </Text>
  ...
  useEffect(() => {
    if (!IS_DEV_ENV && MATOMO_URL != undefined && MATOMO_SITE_ID != undefined) {
      init({
        url: MATOMO_URL,
