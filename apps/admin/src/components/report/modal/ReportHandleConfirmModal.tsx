...
import { HandleReportRequestDto, ReportService } from "@repo/wrapper/server";
import { useMutation } from "@tanstack/react-query";
import { notifications } from "@mantine/notifications";
import HandleAction = HandleReportRequestDto.action;
import { BaseModalProps, useReports } from "@repo/ui";
...
const ReportHandleConfirmModal = ({ reportId, opened, onClose }: Props) => {
  ...
  return (
    <Modal opened={opened} onClose={onClose} title={"Confirm report closing"}>
      <Modal.Body>
        <Stack className={"w-full h-full items-center"}>
          <Select
            ...
            onChange={(v) => {
              if (v) {
                setHandleAction(v as HandleReportRequestDto.action);
                return;
              }
            }}
            data={SELECT_ACTIONS}
