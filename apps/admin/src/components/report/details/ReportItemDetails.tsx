...
const getSourceUrl = async (report: Report) => {
  return await match<ReportSourceType, Promise<string> | string>(
    ...
    .with(ReportSourceType.REVIEW_COMMENT, async () => {
      ...
      const reviewComment = await CommentService.commentControllerFindOneByIdV1(
        commentSourceType,
    ...
    .with(ReportSourceType.POST_COMMENT, async () => {
      ...
      const postComment = (await CommentService.commentControllerFindOneByIdV1(
        commentSourceType,
        report.targetPostCommentId!,
      )) as ActivityComment;
      ...
      <Stack className={"w-full"}>
        <Group className={"w-full justify-around flex-nowrap"}>
          <Paper
            withBorder
            radius={"sm"}
            className={"w-full lg:w-6/12 !bg-[#181818] h-[200px] relative"}
          >
            <Box className={"absolute ms-3 mt-1"}>
              <Text className={"text-sm text-dimmed"}>Report creator</Text>
            </Box>
            <Group className={"w-full h-full px-4 py-2 relative flex-nowrap"}>
              ...
              <Stack className={"w-7/12"}>
                <Group>
                  ...
                  <Tooltip label={reportCategoryToDescription(data.category)}>
                    <Text>{reportCategoryToString(data.category)}</Text>
                  </Tooltip>
                </Group>
                <Group className={"w-fit flex-nowrap"}>
                  <Text fw={"bold"}>Source: </Text>
                  <Text>{reportSourceTypeToString(data.sourceType)}</Text>
                </Group>
                <Group className={"w-fit flex-nowrap"}>
                ...
                <Group className={"w-fit flex-nowrap"}>
                  ...
                  <Badge color={data.isClosed ? "green" : "red"}>
          ...
          >
            <Box className={"absolute ms-3 mt-1"}>
              <Text className={"text-sm text-dimmed"}>Take action</Text>
            </Box>
        ...
        {data.reason != undefined && (
          <Paper
            withBorder
            radius={"sm"}
            className={"w-full !bg-[#181818] min-h-fit h-[240px] relative"}
          >
            <Box className={"absolute ms-3 mt-1"}>
              <Text className={"text-sm text-dimmed"}>Report reason</Text>
            </Box>
        ...
        <Group className={"w-full justify-between flex-nowrap"}>
          <Paper
            withBorder
            radius={"sm"}
            className={"w-full lg:w-4/12 !bg-[#181818] h-[130px] relative"}
          >
            <Box className={"absolute ms-1 mt-1"}>
              <Text className={"text-sm text-dimmed"}>Reported user</Text>
            </Box>
          ...
          >
            <Box className={"absolute ms-3 mt-1"}>
              <Text className={"text-sm text-dimmed"}>View content</Text>
            </Box>
            <Center className={"w-full h-full"}>
              <MainAppLink href={`${sourceUrl}`}>
                <Button loading={sourceUrl == undefined} variant={"outline"}>
